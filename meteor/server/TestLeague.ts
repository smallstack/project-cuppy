import { TestDataGenerator, IOC, Utils, CollectionsService, Collection, User } from "smallstack";
import {
...
Meteor.startup(() => {
    ...
    function createCompetition(displayName: string, type: string): void {
        ...
        let foundCompetition = competitionService.getCompetitionByName<Competition>({ name: name }).val(0);
        if (foundCompetition)
